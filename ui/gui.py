...
def mostrar_resultados():
    ...
    thread = threading.Thread(target=lambda: ejecutar_busqueda(origen, destino, fecha_salida, fecha_regreso, adultos))
    thread.daemon = True
    thread.start()
...
def ejecutar_busqueda(origen, destino, fecha_salida, fecha_regreso, adultos=1):
    global busqueda_en_progreso
    try:
        resultado = controller.ejecutar_busqueda(origen, destino, fecha_salida, fecha_regreso, adultos)

        root.after(0, lambda: actualizar_resultados_ui(resultado))
    except Exception as e:
        root.after(0, lambda: mostrar_error_ui(str(e)))
    finally:
        busqueda_en_progreso = False
...
def actualizar_resultados_ui(resultado):
    """Actualiza la UI con los resultados de la búsqueda (debe ejecutarse en el hilo principal)"""
    resultados_text.config(state=tk.NORMAL)
    resultados_text.delete(1.0, tk.END)
...
def mostrar_error_ui(error_msg):
    """Muestra un error en la UI (debe ejecutarse en el hilo principal)"""
    resultados_text.config(state=tk.NORMAL)
    resultados_text.delete(1.0, tk.END)
...
def aplicar_filtros():
    """Aplica los filtros seleccionados y actualiza la visualización"""
    mostrar_vuelos_filtrados()


def mostrar_vuelos_filtrados():
    """Muestra los vuelos aplicando los filtros actuales"""
    ...
    vuelos_filtrados = controller.obtener_vuelos_filtrados(precio_max, duracion_max, escalas_max)
...
def habilitar_filtros():
    """Habilita los controles de filtro después de obtener resultados"""
...
def limpiar_filtros():
    """Limpia todos los filtros y muestra todos los vuelos"""
...
def toggle_filtro_precio():
    """Habilita/deshabilita el filtro de precio"""
...
def toggle_filtro_duracion():
    """Habilita/deshabilita el filtro de duración"""
...
def toggle_filtro_escalas():
    """Habilita/deshabilita el filtro de escalas"""
...
def resetear_filtros_antes_busqueda():
    """Resetea los filtros y deshabilita controles antes de una nueva búsqueda"""
    precio_habilitado_var.set(False)
    duracion_habilitado_var.set(False)
    escalas_habilitado_var.set(False)
    precio_max_var.set(2000)
...
def iniciar_interfaz():
    ...
    global precio_habilitado_var, duracion_habilitado_var, escalas_habilitado_var
    global precio_max_var, duracion_max_var, escalas_max_var
    global precio_scale, duracion_scale, escalas_spinbox
    global precio_check, duracion_check, escalas_check
    global aplicar_filtros_button, limpiar_filtros_button
    global adultos_var, ordenar_precio_button, ordenar_duracion_button, ordenar_escalas_button
    ...
    tk.Label(inputs_frame, text="Escribe 3+ letras para buscar aeropuertos", font=("Arial", 8, "italic"), fg="gray", bg="#ffffff").grid(row=0,
                                                                                                                column=2,
                                                                                                                sticky="w")
    ...
    tk.Label(inputs_frame, text="Escribe 3+ letras para buscar aeropuertos", font=("Arial", 8, "italic"), fg="gray", bg="#ffffff").grid(row=1,
                                                                                                                column=2,
                                                                                                                sticky="w")
    ...
    tk.Label(herramientas_inner, text="Ordenar por:", font=("Arial", 10, "bold"), bg="#ffffff").pack(side=tk.LEFT, padx=(0, 10))
    ...
    ordenar_duracion_button = tk.Button(herramientas_inner, text="Duración",
                                        command=lambda: [controller.ordenar_vuelos("duracion"), mostrar_vuelos_filtrados()],
    ...
    ordenar_escalas_button = tk.Button(herramientas_inner, text="Escalas",
                                       command=lambda: [controller.ordenar_vuelos("escalas"), mostrar_vuelos_filtrados()],
