...
class FlightController:
    ...
    def ejecutar_busqueda(self, origen, destino, fecha_salida, fecha_regreso, adultos=1, moneda="EUR"):
        """Ejecuta la búsqueda de vuelos y devuelve el resultado"""
        resultado = buscar_vuelos(origen, destino, fecha_salida, fecha_regreso, adultos, moneda)
        ...
        self.busqueda_actual = {
            ...
            "adultos": adultos,
            "moneda": resultado.get("moneda", moneda)
        }
    ...
    def obtener_vuelos_filtrados(self, precio_max=None, duracion_max=None, escalas_max=None, hora_salida_min=None,
        ...
        return filtrar_vuelos(
            ...
            hora_salida_max=hora_salida_max
        )
    ...
    def formatear_vuelos_para_display(self, vuelos_filtrados):
        ...
        moneda = self.busqueda_actual.get('moneda', 'EUR')
        ...
        if stats:
            texto += f"Precio (por persona): Mín: {stats['precio_min']:.2f} {moneda} | "
            texto += f"Máx: {stats['precio_max']:.2f} {moneda} | "
            texto += f"Promedio: {stats['precio_promedio']:.2f} {moneda}\n"

            if adultos > 1:
                texto += f"Precio total ({adultos} adultos): Mín: {stats['precio_min'] * adultos:.2f} {moneda} | "
                texto += f"Máx: {stats['precio_max'] * adultos:.2f} {moneda} | "
        ...
        for vuelo in vuelos_filtrados:
            ...
            texto_vuelo = f"Precio: {precio_unitario:.2f} {moneda}"
            if adultos > 1:
                texto_vuelo += f" | Precio Total ({adultos} adultos): {precio_total:.2f} {moneda}"
